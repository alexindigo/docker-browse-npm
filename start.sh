...
NPM_HOST="${PRIVATE_NPM_PORT_80_TCP_ADDR}"
NPM_PORT="${PRIVATE_NPM_PORT_80_TCP_PORT}"
...
then
  ...
  sed 's%CACHE_PORT%'${CACHE_PORT}'%' _browsenpm.tmp1.json > _browsenpm.tmp2.json
  sed 's%NPM_HOST%'${NPM_HOST}'%' _browsenpm.tmp2.json > _browsenpm.tmp1.json
  sed 's%NPM_PORT%'${NPM_PORT}'%' _browsenpm.tmp1.json > _npm_ui.json
